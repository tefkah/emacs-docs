...
This customization type is similar to `alist` (see above), except that (i) the information is stored as a property list, (see [Property Lists](/docs/elisp/Property-Lists)), and (ii) `key-type`, if omitted, defaults to `symbol` rather than `sexp`.
...
In any alternative for which `nil` is not a valid value, other than a `const`, you should specify a valid default for that alternative using the `:value` keyword. See [Type Keywords](/docs/elisp/Type-Keywords).
