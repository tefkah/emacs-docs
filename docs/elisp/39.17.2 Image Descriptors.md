...
An *image descriptor* is a list which specifies the underlying data for an image, and how to display it. It is typically used as the value of a `display` overlay or text property (see [Other Display Specs](/docs/elisp/Other-Display-Specs)); but See [Showing Images](/docs/elisp/Showing-Images), for convenient helper functions to insert images into buffers.
...
The image type. See [Image Formats](/docs/elisp/Image-Formats). Every image descriptor must include this property.

### `:file file`

This says to load the image from file `file`. If `file` is not an absolute file name, it is expanded relative to the `images` subdirectory of `data-directory`, and failing that, relative to the directories listed by `x-bitmap-file-path` (see [Face Attributes](/docs/elisp/Face-Attributes)).
...
See [Multi-Frame Images](/docs/elisp/Multi_002dFrame-Images).
...
This specifies the pointer shape when the mouse pointer is over this image. See [Pointer Shape](/docs/elisp/Pointer-Shape), for available pointer shapes.
...
This function returns `t` if image `spec` has a mask bitmap. `frame` is the frame on which the image will be displayed. `frame` `nil` or omitted means to use the selected frame (see [Input Focus](/docs/elisp/Input-Focus)).

### <span className="tag function">`function`</span> **image-transforms-p** *\&optional frame*

This function returns non-`nil` if `frame` supports image scaling and rotation. `frame` `nil` or omitted means to use the selected frame (see [Input Focus](/docs/elisp/Input-Focus)). The returned list includes symbols that indicate which image transform operations are supported:
...
If image transforms are not supported, `:rotation`, `:crop`, `:width`, `:height`, `:scale`, `:max-width` and `:max-height` will only be usable through ImageMagick, if available (see [ImageMagick Images](/docs/elisp/ImageMagick-Images)).
