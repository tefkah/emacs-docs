...
The strings are compared by the numeric values of their characters. For instance, `str1` is considered less than `str2` if its first differing character has a smaller numeric value. If `ignore-case` is non-`nil`, characters are converted to upper-case before comparing them. Unibyte strings are converted to multibyte for comparison (see [Text Representations](Text-Representations)), so that a unibyte string and its conversion to multibyte are always regarded as equal.
