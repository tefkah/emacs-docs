...
As in the previous example (see [Example 1](/docs/elisp/pcase_002dexample_002d1)), `and` begins with a `pred` sub-pattern to ensure the following sub-patterns work with an object of the correct type (string, in this case). If `(stringp expval)`<!-- /@w --> returns `nil`, `pred` fails, and thus `and` fails, too.
