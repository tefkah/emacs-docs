...
Prompt for a date, defaulting to the current date. The format of the date is specified by the `TeX-date-format` option. If you want to change the format when the ‘`babel`’ package is loaded with a specific language, set `TeX-date-format` inside the appropriate language hook, for details see [European](European).
