...
By default, Emacs displays text on graphical displays using a 10-point monospace font, and the font size can be changed interactively (see [Text Scale](Text-Scale)).
...
*   Add a line to your init file, modifying the variable `default-frame-alist` to specify the `font` parameter (see [Frame Parameters](Frame-Parameters)), like this:
...
*   Add an ‘`emacs.font`’ X resource setting to your X resource file, like this:
    ...
    You must restart X, or use the `xrdb` command, for the X resources file to take effect. See [Resources](Resources). Do not quote font names in X resource files.
...
*   Use the command line option ‘`-fn`’ (or ‘`--font`’). See [Font X](Font-X).
...
While running Emacs, you can also set the font of a specific kind of text (see [Faces](Faces)), or a particular frame (see [Frame Parameters](Frame-Parameters)).
