...
Before reporting a bug, first try to see if the problem has already been reported (see [Known Problems](Known-Problems)).
...
*   The precise commands we need to type to reproduce the bug. If at all possible, give a full recipe for an Emacs started with the ‘`-Q`’ option (see [Initial Options](Initial-Options)). This bypasses your personal customizations.
...
*   A patch for the bug.
    ...
    See [Sending Patches](Sending-Patches), for guidelines on how to make it easy for us to understand and install your patches.
