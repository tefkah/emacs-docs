...
Many help commands display their information in a special *help buffer*. In this buffer, you can type `SPC` and `DEL` to scroll and type `RET` to follow hyperlinks. See [Help Mode](Help-Mode).
...
This searches for commands whose names match the argument `topics`. The argument can be a keyword, a list of keywords, or a regular expression (see [Regexps](Regexps)). See [Apropos](Apropos).
...
This displays the available Emacs packages based on keywords. See [Package Keywords](Package-Keywords).
...
| • [Help Echo](Help-Echo)               |    | Help on active text and tooltips (“balloon help"). |
